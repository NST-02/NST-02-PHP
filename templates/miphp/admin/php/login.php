...
namespace Templates\MiPHP\Admin\Page;

class Login extends \Templates\MiPHP\Admin\Template
{
    ...
    public function phpfunc()
    {
        if(isset($_POST['ajax']))
        {
            ...
            if($ajax == 'login')
            {
                ...
                {
                    $login = new \Extensions\Mi\Account\Index($this->mi);
                    $loginn = $login->signInForm($username, $password);
                    if ($loginn == false) {
                        $this->alert('danger', 'Hata!', 'Giriş Bilgilerinizi Kontrol Ediniz!');
                    } else {
                        $login->signIn($loginn);
                        $this->alert('success', 'Giriş Başarılı!', 'Yönlendiriliyorsunuz...', false);
                        $this->mi->jsRedirect($this->mi->baseUrl('admin'),2000);
                    }
                }
            }
