...
class Helper extends \Extensions\Prepare
{
    ...
    {
        if($version == 1) $date = date("d.m.Y",strtotime($date));
        if($version == 2) $date = date("d.m.Y H:i",strtotime($date));
        if($version == 3)
        {
    ...
    {
        if($type == 1) $return = mb_convert_case($str, MB_CASE_TITLE, "UTF-8");
        elseif($type == 2)
        {
